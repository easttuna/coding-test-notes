...
# bFS로 상하좌우 바이러스가 퍼질수있는 곳을 면적을 세며 확인
# 2인 곳에서 시작해, 0인곳을 2로 바꾸어 나감
def spread_virus(lab_map):
    n_size = len(lab_map)
    m_size = len(lab_map[0])
    drs = [-1,1,0,0]
    dcs = [0,0,-1,1]
    for r in range(n_size):
        for c in range(m_size):
            ...
            if lab_map[r][c] == 0:
                for dr, dc in zip(drs, dcs):
                    n_r, n_c = r+dr, c+dc
                    if n_r not in range(n_size) or n_c not in range(m_size):
                        continue
                    if lab_map[n_r][n_r] == 2:
                        lab_map[r][c] = 2
                        break
            else:
                continue
